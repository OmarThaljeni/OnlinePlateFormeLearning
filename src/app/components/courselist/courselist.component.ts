...
removeCourse(coursId : string) {
  console.log(coursId);
  this.userService.removeCoureById(coursId).subscribe(() => {
...
visitCourse(coursename : string)
{
  ...
  {
     $("#alertOne").modal('show');
    //window.alert("Vous n'êtes pas inscrit au cours, veuillez l'inscrire pour continuer !!!");
  }
    
}


visitCourseExt(coursename : string, videoId : string)
{

  if(this.enrolledStatus.slice(0, 1).shift() === "enrolled" || this.enrolledStatus2 === "enrolled")
    this._router.navigate(['/fullcourse', coursename,videoId]);
  else if(this.enrolledStatus.slice(0, 1).shift() === "notenrolled")
  {
     $("#alertOne").modal('show');
    //window.alert("Vous n'êtes pas inscrit au cours, veuillez l'inscrire pour continuer !!!");
